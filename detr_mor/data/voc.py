...
#: The 20 VOC foreground classes. A 'background' class is prepended at index 0,
#: giving the 21 classes the configs declare via num_classes.
VOC_CLASSES = [
...
#: Images with more targets than this are dropped, because the model only has
#: num_queries slots to assign them to. On VOC this discards ~15 images.
...
def load_images_and_anns(im_sets, label2idx, ann_fname, split,
    ...
    :param max_objects: images with more objects than this are skipped, since the
        model has a fixed number of queries to match them against
    :return: list of per-image info dicts
    """
    im_infos = []
    for im_set in im_sets:
        ...
        for im_name in im_names:
            ...
            # Because we are using 25 as num_queries, we ignore all images in VOC
            # with more than 25 target objects. This is okay, since it just means
            # we are ignoring a small number of images (15, to be precise).
...
class VOCDataset(Dataset):
    ...
    Returns (im_tensor, targets, filename) where:
      * im_tensor is (3, im_size, im_size) float32, ImageNet-normalised
      * targets['boxes'] is (N, 4) x1y1x2y2 normalised to [0, 1]
      * targets['labels'] is (N,) int64, 0 == background
      * targets['difficult'] is (N,) int64, used only by mAP evaluation

    :param split: 'train' (reads trainval.txt, augments) or 'test' (reads test.txt)
    :param im_sets: list of VOCdevkit/VOC20xx roots
    :param im_size: images are resized to a square of this size
    :param verbose: print the label mapping on construction
    """

    def __init__(self, split, im_sets, im_size=640, verbose=True):
        self.split = split
        ...
        classes = ['background'] + sorted(VOC_CLASSES)
